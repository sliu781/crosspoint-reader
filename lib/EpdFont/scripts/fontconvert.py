...
parser.add_argument("--output", dest="output", default=None, help="Write a binary .cpfont file to this path instead of a C header to stdout. Requires uncompressed 1-bit font (no --compress or --2bit).")
...
if args.output:
    # Write binary .cpfont instead of C header.
    # Only supports uncompressed 1-bit fonts (no --compress, no --2bit).
    if compress or is2Bit:
        raise ValueError("--output binary mode requires uncompressed 1-bit font (no --compress, no --2bit)")
    import struct as _s, sys as _sys
    _kl  = len(kern_left_classes)  if kern_map else 0
    _kr  = len(kern_right_classes) if kern_map else 0
    _klc = kern_left_class_count   if kern_map else 0
    _krc = kern_right_class_count  if kern_map else 0
    _lp  = len(ligature_pairs)     if ligature_pairs else 0
    _gc  = sum(e - s + 1 for s, e in intervals)
    buf = bytearray()
    buf += b'CPFT'
    buf += _s.pack('B',  1)
    buf += _s.pack('B',  norm_ceil(face.size.height))
    buf += _s.pack('<i', norm_ceil(face.size.ascender))
    buf += _s.pack('<i', norm_floor(face.size.descender))
    buf += _s.pack('B',  0)
    buf += _s.pack('<I', len(intervals))
    buf += _s.pack('<I', _gc)
    buf += _s.pack('<I', len(glyph_data))
    buf += _s.pack('<H', _kl)
    buf += _s.pack('<H', _kr)
    buf += _s.pack('B',  _klc)
    buf += _s.pack('B',  _krc)
    ...
    for s, e in intervals:
        buf += _s.pack('<III', s, e, _off); _off += e - s + 1
    for g in glyph_props:
        buf += _s.pack('<BBHhhHI', g.width, g.height, g.advance_x, g.left, g.top, g.data_length, g.data_offset)
    buf += bytes(glyph_data)
    if kern_map:
        for cp, cls in kern_left_classes:  buf += _s.pack('<HB', cp, cls)
        for cp, cls in kern_right_classes: buf += _s.pack('<HB', cp, cls)
        for v in kern_matrix:              buf += _s.pack('b', v)
    if ligature_pairs:
        for pair, lig in ligature_pairs:   buf += _s.pack('<II', pair, lig)
    with open(args.output, 'wb') as _f: _f.write(buf)
    print(f"Wrote {len(buf)} bytes to {args.output}", file=_sys.stderr)
    _sys.exit(0)
